...
for (gbid in unique(data$ID)) {
  ...
  print(summary(factor(d$value)))

  if (nrow(d) == 0) { next }
  # add explicit warning checks to make debugging easier.
  not_in_tree <- setdiff(d$variable, tree$tip.label)
  ...
  for (model in c("ER", "ARD")) {
    ...
    r <- corHMM::rayDISC(
      tree, d, ntraits=1, charnum=1, model=model,
...
write.csv(rates, sprintf('asr_rates.%s.csv', treeid), quote=FALSE, row.names=FALSE)
write.csv(states, sprintf('asr_states.%s.csv', treeid), quote=FALSE, row.names=FALSE)
